...
namespace Bimangle.ForgeEngine.Revit.Core
{
    class LicenseConfig
    {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        public static LicenseSessionX Create(Autodesk.Revit.ApplicationServices.Application revitApp, string licenseKey = null)
        {
            //从自定义授权文件加载授权
            licenseKey = GetLicenseKey(licenseKey);

            return new LicenseSessionX(revitApp, CLIENT_ID, APPLICATION_NAME, licenseKey);
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        public static void ShowDialog(LicenseSessionX session, IWin32Window parent)
        {
            LicenseSessionX.ShowLicenseDialog(session.RevitApp, session.ClientId, session.AppName, parent, DeployLicenseFile);
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        public static void ShowDialog(Autodesk.Revit.ApplicationServices.Application revitApp, IWin32Window parent)
        {
            //从自定义授权文件加载授权
            var licenseKey = GetLicenseKey(null);
            LicenseSessionX.GetLicenseInfo(CLIENT_ID, APPLICATION_NAME, licenseKey);

            LicenseSessionX.ShowLicenseDialog(revitApp, CLIENT_ID, APPLICATION_NAME, parent, DeployLicenseFile);
        }
